...
namespace ClassicalComponentsDemo.iOS
{
    public class BusinessCard : NSObject
    {
        ...
        public BusinessCard(UIImage image, string recognizedText)
        {
            Image = image;
    ...
    public partial class BusinessCardsDemoViewController :
        UIViewController, ISBSDKMultipleObjectScannerViewControllerDelegate, IUICollectionViewDelegate, IUICollectionViewDataSource
    {
        ...
        {
            if (segue.DestinationViewController is BusinessCardDetailDemoViewController)
            {
                ...
                destinationViewController.BusinessCard = _selectedCard;
            }
        }

        private void ProcessDetectedDocumentsInStorage(ISBSDKImageStoring storage)
        {
            ShowLoadingView();
            _isProcessing = true;
            _imageProcessor.ProcessImageStorage(storage, "en+de", (processedStorage, results) =>
            {
                _storage = processedStorage;
                _ocrResults = results;
        ...
        private void CloseCollectionContainer()
        {
        ...
        {
            BusinessCardDemoCell cell = (BusinessCardDemoCell)collectionView
                .DequeueReusableCell("BusinessCardCell", indexPath);
            cell.Image = _storage.ImageAtIndex((nuint)indexPath.Row);
            return cell;
        }

        public nint GetItemsCount(UICollectionView collectionView, nint section)
        {
            return (nint)_storage.ImageCount;
        }
        ...
        {
            UIImage image = _storage.ImageAtIndex((nuint)indexPath.Row);
            string text = _ocrResults?.Count > 0 ? _ocrResults[indexPath.Row].RecognizedText : "";
            _selectedCard = new BusinessCard(image, text);
            return true;
        }
        ...
        [Export("scannerController:didCaptureObjectImagesInStorage:")]
        public void ScannerController(SBSDKMultipleObjectScannerViewController controller,
            ISBSDKImageStoring imageStorage)
        {
            ProcessDetectedDocumentsInStorage(imageStorage);
        }

        [Export("scannerControllerWillCaptureImage:controller")]
        public void ScannerControllerWillCaptureImage(SBSDKMultipleObjectScannerViewController controller)
        {
            _storage.RemoveAllImages();
        }

        [Export("scannerControllerShouldAnalyseVideoFrame:controller")]
        public bool ScannerControllerShouldAnalyseVideoFrame(SBSDKMultipleObjectScannerViewController controller)
        {
            bool shouldAnalyse = true;
            InvokeOnMainThread(() =>
            {
